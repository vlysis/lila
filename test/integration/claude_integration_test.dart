...
void main() {
  ...
  group('Spec Test Case 1: Key stored in secure enclave', () {
    test('key is stored in secure storage, not SharedPreferences', () async {
      ...
      await service.saveApiKey(AiProvider.claude, testKey);

      // Key should be in secure storage
      expect(mockSecureStorage['lila_ai_api_key_claude'], equals(testKey));

      // Key should NOT be in SharedPreferences
      final prefs = await SharedPreferences.getInstance();
      expect(prefs.getString('lila_ai_api_key_claude'), isNull);
      expect(prefs.getString('claude_api_key'), isNull);
    });

    test('key is retrievable only via ClaudeService', () async {
      const testKey = 'sk-ant-api03-abcdefghijklmnopqrstuvwxyz0123456789ABCD';
      mockSecureStorage['lila_ai_api_key_claude'] = testKey;

      final service = await AiIntegrationService.getInstance();
      final retrievedKey = await service.getApiKey(AiProvider.claude);
  ...
  group('Spec Test Case 6: Key masked after save, clipboard cleared', () {
    test('key is masked in display', () async {
      ...
      final maskedKey = service.maskedKey(AiProvider.claude);
  ...
  group('Spec Test Case 8: Rate limit triggers back-off', () {
    test('429 is in retryable status codes', () {
      ...
      expect(AiApiError.rateLimited.userMessage, contains('usage limit'));
    });

    test('401 is NOT retried (non-retryable)', () {
      // 401 should fail immediately, not retry
      expect(AiApiError.keyInvalid.userMessage, contains('invalid'));
    });
  });
  ...
  group('Daily cap integration', () {
    test('cap blocks requests when reached', () async {
      ...
      expect(result.isError, isTrue);
      expect(result.error, equals(AiApiError.dailyCapReached));
    });
  });
  ...
  group('Model selection integration', () {
    test('model persists across service instances', () async {
      final service1 = await AiIntegrationService.getInstance();
      await service1.setModel(AiProvider.claude, 'claude-sonnet-4-20250514');

      AiIntegrationService.resetInstance();

      // Need to preserve SharedPreferences state
      final prefs = await SharedPreferences.getInstance();
      final model = prefs.getString('ai_model_claude');
      if (model != null) {
        SharedPreferences.setMockInitialValues({
          'ai_model_claude': model,
        });
      }

      final service2 = await AiIntegrationService.getInstance();
      expect(
        service2.selectedModel(AiProvider.claude),
